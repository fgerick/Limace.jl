...
def runKoreRes(Rac,opts): # Print residuals once Rac is found
    ...
    os.system("sed -i '' 's/^Ra_gap=.*/Ra_gap=%f/' ./bin/parameters.py" %Ra)
...
def get_sigma(Ra,ncpus, opts):
    ...
    if Ra in ra_cache:
        ...
        sigma_c = eig[Idx,0]
        ra_cache[Ra] = sigma_c
        print("σ_c = %e" %sigma_c,flush=True)
...
for m in marr:
    ...
    os.system("sed -i '' 's/^m =.*/m = %d/' ./bin/parameters.py" %m)
    print("sed1 done",flush=True)
    os.system('./bin/submatrices.py %d > /dev/null' %par.ncpus)
    print("submat",flush=True)
    Rac = bracket_brentq(get_sigma,np.log10(Ramin),np.log10(Ramax), args=(par.ncpus,opts))
