...
@testset "Free decay modes BC explicit" begin
    ...
    function getlk1(Nmax, lmax, TB)
        b = TB(Nmax; m=0)
        LHS = Limace.inertial(b; external=false)
        RHS = Limace.diffusion(b; external=false)
		_n = norm(LHS)
		LHS/=_n
		RHS/=_n
        found = [
            (first(first(eigstarget(RHS, LHS, λfd(l, k) + 1e-3, nev = 1))), λfd(l, k)) for
    ...
    #testing:
    # fs = getlk1(40, 4, Insulating)
    fs = getlk1(20, 4, Limace.InsulatingNoBC)

    for (f_num, f_λfd) in fs
        @test f_num ≈ f_λfd
    end
